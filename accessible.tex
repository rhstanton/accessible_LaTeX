...
\item Included in TeX Live 2023+ (no separate installation needed)
...
\begin{frame}{Important workflow change: Use LuaLaTeX}
  ...
  \item \tb{Why switch to LuaLaTeX?}
    \begin{enumerate}
    \item \textbf{Automatic MathML:} LuaLaTeX automatically generates \hrefu{https://www.w3.org/Math/}{MathML} (Mathematical Markup Language) for equations, making math accessible to screen readers without extra work.
...
\begin{frame}{Common Pitfalls to Avoid}
  ...
  \item \tr{Not specifying table header rows}
    \begin{itemize}
    \item Add \texttt{\textbackslash tagpdfsetup\{table/header-rows=\{1\}\}} before each data table
    \item Forget this and screen readers can't navigate your tables properly
    \end{itemize}
    
  \item \tr{Using the wrong compiler}
    \begin{itemize}
    \item Make sure your editor is set to LuaLaTeX, not pdfLaTeX
