...
\renewcommand\@framenumber{%
  \ifnum\value{page}=1\relax
    ...
    \hspace*{-.2in}%
    \raisebox{0.0in}[.25in][.0in]{\includegraphics[width=.45in]{BerkeleyHaas}}
...
\item Both require LaTeX kernel 2025-11-01 (see next slides for details)
...
%\item The \texttt{tagging-setup} entries keep math accessible while reducing helper attachments that can trigger warnings in some strict conformance validators.
...
\begin{itemize}
\item The metadata settings above prevent most embedded helper files used by tagging internals.
\item Some PDFs may still contain /AF or embedded-file structures that some strict conformance checkers flag.
...
\item Prerequisite: install the Python package first with \texttt{python -m pip install pikepdf}.
\end{itemize}
...
\begin{minipage}{0.9\linewidth}
\ttfamily\small
python strip\_af.py build/accessible\_slides.pdf build/accessible\_slides.noaf.pdf
\end{minipage}
}
...
\begin{itemize}
\item Output: \texttt{build/accessible\_slides.noaf.pdf}
\item This keeps document content/tagging while removing AF and embedded-file metadata.
\end{itemize}
...
\begin{frame}{Getting started: Common steps for both}
  ...
  \item \textbf{Compile (plus optional validator cleanup):}
    ...
    \item If needed, install dependency first: \texttt{python -m pip install pikepdf}
    \item Optional: run \texttt{python strip\_af.py build/yourfile.pdf build/yourfile.noaf.pdf} before strict validation in some conformance checkers
    \end{itemize}
  \end{enumerate}
...
\begin{itemize}
\item \textbf{Checkers used so far:} Ally (the accessibility checker built into bCourses) and veraPDF (open-source PDF conformance validator, \urlu{https://verapdf.org/}).
\end{itemize}
\end{frame}
