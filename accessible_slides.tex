...
\AtBeginDocument{
  ...
  %          but section titles are often empty (just organizational markers).
  %          PDF validators like Acrobat complain about H1 tags with no text.
  %
  % Solution: Disable tagging for section titles; use H1 for frame titles instead.
  %          Frame titles always have content (they're the slide titles).
  %
  % Technical details:
  % - \NewTaggingSocketPlug creates a "none" plug that does nothing
  % - \AssignTaggingSocketPlug applies it to the talk/sec/title socket
  % - This prevents empty H1 tags from being generated for \section commands
  % - Frame titles become H1 instead (see \tagpdfsetup below)
...
% This works together with the section title fix above:
% - Sections are used for organization but don't create tags
% - Frame titles become H1 (the top-level heading in the document)
% - Screen readers use heading levels to navigate slides
...
\begin{itemize}
\item \tg{Minimum:} TeX Live 2023 or later (current = 2026) with all packages updated
...
\begin{itemize}
\item
Including tables is the same as in Beamer (e.g., using the \texttt{tabular} environment).
%\begin{itemize}
...
\begin{frame}{Validator-specific notes}
  ...
  \item \textbf{PAC: ``Table header cell has no associated subcells''}
    \begin{itemize}
    \item Often a validator interpretation issue, not a source error
    \item Verify \texttt{table/header-rows} is correct first
    \item PAC may not fully handle attribute-class form for table scope
    \item Reference: \urlu{https://github.com/latex3/tagging-project/issues/1056}
    \end{itemize}

  \item \textbf{Bottom line}
    \begin{itemize}
    \item Use multiple validators (Ally, veraPDF, PAC) and compare findings
