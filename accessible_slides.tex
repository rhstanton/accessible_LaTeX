...
%    \hspace*{-3in}%
    ...
    \hfill \hspace*{-.65in} Richard Stanton, \the\year \hfill 
  \else
...
\hspace*{0.2in}pdfversion=1.7,\hspace*{0.35in}\% Explicit PDF version\\[0.5ex]
\hspace*{0.2in}lang=en-US,\hspace*{0.73in}\% Language for screen readers\\[0.5ex]
\hspace*{0.2in}tagging=on,\hspace*{0.78in}\% Enable PDF tagging\\[0.5ex]
\hspace*{0.2in}tagging-setup=\{\\[0.5ex]
\hspace*{0.4in}math/alt/use,\hspace*{0.2in}\% Keep math accessibility enabled\\[0.5ex]
...
\begin{minipage}{0.9\linewidth}
\ttfamily\small
python build/strip\_af.py accessible\_slides.pdf
\end{minipage}
}
...
\begin{itemize}
\item Output: \texttt{accessible\_slides.noaf.pdf}
...
\begin{frame}{Getting started: Common steps for both}
  ...
  \item \textbf{Compile (plus optional validator cleanup):}
    ...
    \item Optional: run \texttt{python build/strip\_af.py yourfile.pdf} before strict validation in some conformance checkers
    \end{itemize}
  \end{enumerate}
\end{frame}
