...
\noindent \textbf{One Ally-quirk workaround.} The preamble also calls \texttt{\textbackslash tagpdfsetup\{math/alt/use\}} (separately from \texttt{\textbackslash DocumentMetadata}). Under PDF/UA-2 the kernel does not auto-attach \texttt{/Alt} to math \texttt{Formula} tags --- the embedded MathML is meant to suffice --- but Blackboard Ally still checks for \texttt{/Alt} and reports every inline equation as ``image without description'' when it is missing. Opting in silences that false positive.
