...
\title{Accessible Documents in LaTeX \\ \normalsize \urlu{https://github.com/rhstanton/accessible_LaTeX} \\ \ \\ \large Version 1.3}
...
\item \textbf{Compile (plus optional validator cleanup):}
  \begin{itemize}
  \item Compile with LuaLaTeX
  \item Optional (strict archival validation): run \texttt{python strip\_af.py build/yourfile.pdf build/yourfile.noaf.pdf}
  \item Validate the \texttt{.noaf.pdf} output with \texttt{veraPDF}
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
\item \textbf{Different validators check different things}
  \begin{itemize}
  \item Accessibility usability checks and strict PDF conformance checks are not identical
  \item A document can pass one tool and still show warnings in another
  \end{itemize}

\item \textbf{veraPDF + \texttt{strip\_af.py}}
  \begin{itemize}
  \item \texttt{veraPDF} enforces strict PDF/A conformance requirements
  \item If strict archival conformance is required, run \texttt{strip\_af.py} before validating
  \item For many course-delivery contexts, this cleanup is optional
  \end{itemize}
...
\item \textbf{Recommended workflow}
  \begin{itemize}
  \item Keep source tagging correct first (alt text, table header rows, language, tagging)
  \item Use multiple checks (Ally + veraPDF + PAC)
  \item Treat one-tool warnings as prompts to investigate, not automatic proof of source errors
  \end{itemize}
\end{itemize}
