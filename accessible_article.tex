...
\title{Accessible Documents in LaTeX \\ \normalsize \urlu{https://github.com/rhstanton/accessible_LaTeX} \\ \ \\ \large Version 2.0.1}
...
\noindent Template v2.0 targets the newer \BlueText{PDF/UA-2 + PDF/A-4} (PDF~2.0) profile. Most accessibility validators were calibrated for the older \BlueText{UA-1 / PDF~1.7} profile and haven't fully caught up. Ally will report one \emph{false} error: ``The PDF does not have a title'' --- Ally checks a PDF-1.x key that PDF~2.0 deprecates, even though the title is correctly present in XMP. The PDF passes \BlueText{veraPDF} cleanly; see ``Validator-specific notes'' below and \texttt{VALIDATION.md}.

\smallskip

\noindent\RedText{\textbf{Do not accept Ally's offered ``fix'' for this finding.} It may downgrade the file to PDF~1.7, destroying PDF/UA-2 and PDF/A-4 conformance.}
\end{minipage}}
\end{center}
...
\noindent \textbf{One Ally-quirk workaround.} The preamble also calls \texttt{\textbackslash tagpdfsetup\{math/alt/use\}} (separately from \texttt{\textbackslash DocumentMetadata}). Under PDF/UA-2 the kernel does not attach \texttt{/Alt} to math \texttt{Formula} tags by default --- the embedded MathML is meant to suffice --- but Blackboard Ally still checks for \texttt{/Alt} and reports every inline equation as ``image without description'' when it is missing. Opting in silences that false positive.
