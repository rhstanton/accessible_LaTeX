...
\title{Accessible Documents in LaTeX \\ \normalsize \urlu{https://github.com/rhstanton/accessible_LaTeX} \\ \ \\ \large Version 2.1.0}
...
\noindent\textbf{\large If Ally doesn't score this PDF 100\%, don't panic --- that's expected.}

\smallskip

\noindent Template v2.0 targets the newer \BlueText{PDF/UA-2 + PDF/A-4} (PDF~2.0) profile. Most accessibility validators were calibrated for the older \BlueText{UA-1 / PDF~1.7} profile and haven't caught up, so \textbf{Ally will report a score below 100\%}. That reflects checker lag, not real accessibility problems. Ally raises \emph{several false} errors: ``The PDF does not have a title'' (the title \emph{is} present, in the XMP metadata that PDF~2.0 uses), and ``Image without a description'' on \emph{every} equation (the math is tagged as \BlueText{MathML}, which Ally doesn't check for). The PDF passes \BlueText{veraPDF} --- the meaningful UA-2 / PDF/A-4 check --- cleanly; see ``Validator-specific notes'' below and \texttt{VALIDATION.md}.

\smallskip

\noindent\RedText{\textbf{Do not accept Ally's offered ``fixes'' to raise the score} --- they make the PDF \emph{less} accessible, not more. See ``Validator-specific notes'' below and \texttt{VALIDATION.md}.}
\end{minipage}}
\end{center}
...
\noindent \textbf{An Ally finding we deliberately leave unfixed.} Blackboard Ally reports every inline equation as ``image without description.'' Under PDF/UA-2 the kernel attaches \emph{MathML} (not \texttt{/Alt}) to each \texttt{Formula} tag, and that MathML is what carries the math semantics to a screen reader; Ally simply checks for \texttt{/Alt} and has no PDF/UA-2 support yet, so it misreports the formula as undescribed. \textbf{Do not silence this with \texttt{\textbackslash tagpdfsetup\{math/alt/use\}}} (earlier versions of this template did). That attaches the raw \LaTeX{} source as \texttt{/Alt}, and per PDF/UA a screen reader reads an element's \texttt{/Alt} \emph{instead of} descending into its children --- so the \texttt{/Alt} \emph{hides} the MathML, and the user hears the verbatim \LaTeX{} read aloud instead of the rendered equation. Treat the Ally finding as a false positive. See \texttt{VALIDATION.md}.
...
\begin{itemize}
\item The metadata settings produce a fully tagged PDF that passes veraPDF (PDF/A-4 + UA-2). \textbf{Heads up:} Ally currently reports one false error under PDF/UA-2 (legacy PDF-1.x title check). See ``Validator-specific notes'' below.
...
\fcolorbox{AccessibleRed}{red!8!white}{%
  \begin{minipage}{0.85\linewidth}\centering
    \RedText{\large\textbf{Do NOT accept Ally's offered ``fix'' for this finding.}}\\[2pt]
