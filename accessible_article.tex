...
\title{Accessible Documents in LaTeX \\ \normalsize \urlu{https://github.com/rhstanton/accessible_LaTeX} \\ \ \\ \large Version 1.4}
...
\begin{itemize}
\item \tg{\textbf{Your existing \LaTeX\ skills transfer}} - you're just adding accessibility features
\item \tg{\textbf{The changes are minimal}} and follow consistent patterns
\end{itemize}
...
\noindent Both require LaTeX kernel 2025-11-01 (see next sections for details). 
...
\subsection{Optional validator cleanup for some checkers (\texttt{strip\_af.py})}

\begin{itemize}
\item Even with the metadata settings above, some builds may still include PDF Associated File entries.
\item These are typically harmless helper attachments, but some strict conformance checkers can still flag them.
\item Ally can still score 100\% without this step; run \texttt{strip\_af.py} when you need checker-specific conformance cleanup.
\end{itemize}
...
\noindent \fbox{%
{\ttfamily\small
python build/strip\_af.py accessible\_article.pdf
}
}

\bigskip

\noindent This writes \texttt{accessible\_article.noaf.pdf}, removing /AF and embedded-file structures while preserving document content and accessibility tagging.
...
\noindent \textbf{Not all validator warnings indicate real problems.} Some are \textit{validator bugs}:

  \begin{itemize}
  \item \textbf{Acrobat: ``Lbl and LBody -- Failed''} 
    \begin{itemize}
    \item Appears around figure captions, section numbers, table captions
    \item \tg{This is a validator bug}, not a source error
    \item The \texttt{<Lbl>} tag is correctly used for numbering
    \item Acrobat's checker incorrectly expects it only in lists
    \item \textbf{Safe to ignore} -- your PDF structure is correct
    \item References:
      \begin{itemize}
      \item LaTeX developer Ulrike Fischer: ``Adobe doesn't like the (valid) use of Lbl for the section numbers, complain to them that they should correct their checker.'' (\urlu{https://tex.stackexchange.com/a/709655/2388})
      \item Adobe Community expert Philip Kiff: ``<Lbl> is getting flagged as an error by Acrobat Pro DC by mistake... you could just ignore this error and wait until Acrobat fixes their built-in tester.''
      \end{itemize}
    \end{itemize}
    
  \item \textbf{PAC: ``Table header cell has no associated subcells''}
    \begin{itemize}
    \item Often a validator interpretation issue
    \item Verify your \texttt{\textbackslash tagpdfsetup\{table/header-rows=\{...\}\}} is correct
    \item PAC may not fully handle attribute-class form for table scope
    \item Known behavior: \urlu{https://github.com/latex3/tagging-project/issues/1056}
    \end{itemize}
  \end{itemize}

\noindent \textbf{Recommendation:} Use multiple validators (Ally, veraPDF, PAC) and compare findings. No single checker catches everything, and each may have quirks.
...
\noindent \textbf{Checkers used so far:} Ally (the accessibility checker built into bCourses), veraPDF (open-source PDF conformance validator, \urlu{https://verapdf.org/}), and PAC (PDF Accessibility Checker, \urlu{https://pac.pdf-accessibility.org/}).
...
\begin{quote}\ttfamily
python strip\_af.py build/yourfile.pdf build/yourfile.noaf.pdf
\end{quote}

\begin{itemize}
\item Validate \texttt{build/yourfile.noaf.pdf} with \texttt{veraPDF}.
...
\begin{itemize}
\item \textbf{This is often a validator interpretation issue, not necessarily a source-tagging error.}
